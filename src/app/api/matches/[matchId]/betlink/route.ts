...
export async function GET(
  ...
  try {
    ...
    console.log(`Betlink-Anfrage für Match ${matchId} mit Token: ${token.substring(0, 15)}...`);
    ...
    // Anfrage an den Betlink-Endpunkt
    const betlinkResult = await safeApiRequest(`https://api.kickbase.com/v4/matches/${matchId}/betlink`, {
      method: 'GET',
      headers: {
    ...
    if (betlinkResult.error) {
      console.error(`Fehler beim Abrufen des Betlinks: ${betlinkResult.error}`);
      return NextResponse.json(
        { message: `Fehler beim Abrufen des Betlinks: ${betlinkResult.error}` },
        { status: betlinkResult.status || 500 }
      );
    }
    
    const betlinkData = betlinkResult.data;
    console.log('Betlink-Daten erhalten:', Object.keys(betlinkData));
    
    // Daten zurückgeben
    return NextResponse.json(betlinkData);
    
  } catch (error: any) {
    console.error('Betlink fetch error:', error);
    return NextResponse.json(
