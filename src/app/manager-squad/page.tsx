...
function ManagerSquadContent() {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Fehler in manager-squad init:', e);
      setError('Konnte Kader-Seite nicht initialisieren.');
    } finally {
      setIsLoading(false);
    }
  }, [router, searchParams]);

  if (isLoading) return <div className="p-6">Lade Kader…</div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 dark:from-green-950 dark:to-blue-950">
      <header className="bg-white dark:bg-gray-800 shadow">
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">
            Manager-Kader – {league.name ?? league.id}
          </h1>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <p className="text-sm text-gray-700 dark:text-gray-300">Platzhalter – Kaderdaten folgen.</p>
        </div>
      </main>
    </div>
  );
...
export default function ManagerSquadPage() {
  return (
    <Suspense fallback={<div className="p-6">Lade Kader…</div>}>
      <ManagerSquadContent />
    </Suspense>
  );
}
